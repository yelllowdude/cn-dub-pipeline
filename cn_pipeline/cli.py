...
def cmd_review_auth(args):
    ...
    if not args.redirect_url:
        ...
        print("Frame.io authenticates as the SHARED team account. Before signing in:")
        print("  if another operator's `cn-pipeline doctor` shows `frameio token  minted ok`,")
        print("  adopt their token instead -- `cn-pipeline team import --file <bundle> --overwrite`.")
        print("  Signing in as your own Adobe login replaces the team's token with one bound")
        print("  to your session, which is how the team ends up locked out.\n")
        print("If you do need to sign in, sign in AS THE SHARED FRAME.IO ACCOUNT.\n")
        print("!! The code Adobe returns expires in about ONE MINUTE and is SINGLE-USE. !!")
        print("   Have this terminal ready to paste before you open the link, and get a")
        print("   fresh URL for every attempt -- a reused code fails with the same")
        print("   `access_denied` as an expired one, which makes it look like the")
        print("   credential is broken when it isn't.\n")
        ...
        print("3) IMMEDIATELY copy the FULL redirected URL and re-run:\n")
