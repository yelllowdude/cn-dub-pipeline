...
check("HARD cap: no cue ever exceeds the limit", all(len(c) <= ZH_MAX_CHARS for c in cues))
check("nothing lost in the split", "".join(cues) == long_sentence)

no_comma = "这一段完全没有任何标点可以用来断行所以必须在达到上限的时候被强制切开否则字幕会跑出屏幕外面去了"
cues = split_zh_cues(no_comma)
check("comma-less stretch force-breaks at the cap", all(len(c) <= ZH_MAX_CHARS for c in cues) and len(cues) >= 2)
check("force-break loses nothing", "".join(cues) == no_comma)

colon = "而且耐力掉得比力量快——这一点没人提醒你。"
cues = split_zh_cues(colon)
check("dash/colon becomes its own break when over cap",
      all(len(c) <= ZH_MAX_CHARS for c in cues))
